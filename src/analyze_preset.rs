...
mod tests {
    ...
    fn build_preset(ssn_lfo_freq: f32, ssn_lfo_depth: f32) -> Preset {
        let mut objects: Vec<ObjectConfig> = (0..MAX_OBJECTS)
            .map(|_| ObjectConfig::default())
            .collect();

        objects[0] = ObjectConfig {
            active: true, color: 2, // brown vol 0.5
            x: 0.0, y: 0.0, z: 0.0, volume: 0.5, reverb_send: 0.0,
        ...
        objects[1] = ObjectConfig {
            active: true, color: 4, // grey vol 0.7, static at x:1, depth 0.8
            x: 1.0, y: 0.0, z: 1.0, volume: 0.7, reverb_send: 0.0,
        ...
        objects[4] = ObjectConfig {
            active: true, color: 4, // grey vol 0.7, static at x:-1
            x: -1.0, y: 0.0, z: 1.0, volume: 0.7, reverb_send: 0.0,
            bass_mod: ModConfig { kind: 4, param_a: 30.0, param_b: 0.8, param_c: 0.0 },
            satellite_mod: ModConfig { kind: 4, param_a: 30.0, param_b: 0.8, param_c: 0.0 },
            movement: MovementConfig::default(),
        ...
        objects[2] = ObjectConfig {
            active: true, color: 6, // SSN vol 1.0, neuralLfo bass + stochastic sat
            x: 0.0, y: 0.0, z: 1.0, volume: 1.0, reverb_send: 0.0,
            bass_mod: ModConfig { kind: 4, param_a: ssn_lfo_freq, param_b: ssn_lfo_depth, param_c: 0.0 },
            satellite_mod: ModConfig { kind: 3, param_a: 4.0, param_b: 150.0, param_c: 0.85 },
        ...
        objects[3] = ObjectConfig {
            active: true, color: 1, // pink vol 0.5, flat mods
            x: 0.0, y: 0.0, z: 2.0, volume: 0.5, reverb_send: 0.0,
            bass_mod: ModConfig { kind: 0, param_a: 0.0, param_b: 0.0, param_c: 0.0 },
            satellite_mod: ModConfig { kind: 0, param_a: 0.0, param_b: 0.0, param_c: 0.0 },
            movement: MovementConfig::default(),
        };
        ...
        let mut preset = Preset {
            ...
            anchor_color: 0, anchor_volume: 0.0,
    ...
    fn sweep_grey_lfo_depth() {
        ...
        for &depth in &[0.2_f32, 0.4, 0.6, 0.8, 1.0] {
            ...
            let mut objects: Vec<ObjectConfig> = (0..MAX_OBJECTS)
                ...
                .collect();
            objects[0] = ObjectConfig {
                active: true, color: 2, x: 0.0, y: 0.0, z: 0.0, volume: 0.5,
                reverb_send: 0.0,
                bass_mod: ModConfig { kind: 0, param_a: 0.0, param_b: 0.0, param_c: 0.0 },
                satellite_mod: ModConfig { kind: 0, param_a: 0.0, param_b: 0.0, param_c: 0.0 },
                movement: MovementConfig::default(),
            ...
            objects[2] = ObjectConfig {
                ...
                reverb_send: 0.0,
                bass_mod: ModConfig { kind: 4, param_a: 14.0, param_b: 0.2, param_c: 0.0 },
                satellite_mod: ModConfig { kind: 3, param_a: 4.0, param_b: 150.0, param_c: 0.85 },
                movement: MovementConfig {
                    kind: 5, radius: 2.0, speed: 0.4, phase: 0.0,
                    depth_min: 0.5, depth_max: 5.0, reverb_min: 0.0, reverb_max: 0.0,
            ...
            let mut preset = Preset {
                master_gain: 0.8, spatial_mode: 1, source_count: 4,
                anchor_color: 0, anchor_volume: 0.0, environment: 1, objects,
            };
            preset.clamp();

            let r = evaluate_preset(&preset, &goal, &config);
            println!("  {:.1}     {:.4}   {:.3}   {:.3}   {:.3}   {:.3}   {:.4}",
                depth, r.score, r.delta_power, r.theta_power,
                r.alpha_power, r.beta_power, r.gamma_power);
        }
    }
    ...
    fn analyze_sleep_sanctuary_sleep_detail() {
        ...
        for bt in BrainType::all() {
            let config = SimulationConfig { brain_type: *bt, ..SimulationConfig::default() };
            let goal = Goal::new(GoalKind::Sleep);
            let result = evaluate_preset(&preset, &goal, &config);
            println!("Sleep + {:?}: score={:.4}  dom={:.2}Hz  firing={:.2}Hz  ISI_CV={:.4}",
