...
mod tests {
    ...
    fn bilateral_symmetric_input_matches_tonotopic_direction() {
        ...
        assert!(mono.band_powers.total() > 0.0, "mono should produce non-zero EEG");
        assert!(bi.combined.band_powers.total() > 0.0, "bilateral should produce non-zero EEG");

        // Dominant frequency should be in physiological range (1-50 Hz)
        assert!(mono.dominant_freq >= 1.0 && mono.dominant_freq <= 50.0,
            "mono dominant freq {} out of range", mono.dominant_freq);
    ...
    fn bilateral_asymmetric_input_differentiates_hemispheres() {
        ...
        let left_bands: [Vec<f64>; 4] = [
            vec![0.8; n], vec![0.3; n], vec![0.1; n], vec![0.05; n],
        ];
        let left_energy = [0.70, 0.20, 0.08, 0.02];

        // Right ear: strong high-frequency energy (white-like)
        let right_bands: [Vec<f64>; 4] = [
            vec![0.2; n], vec![0.4; n], vec![0.6; n], vec![0.8; n],
        ];
        ...
        assert!(
            ...
            bi.right_dominant_freq, bi.left_dominant_freq
        );

        // Both hemispheres should produce valid output
        assert!(bi.left_band_powers.total() > 0.9,
            "left hemisphere band powers should sum to ~1.0");
        assert!(bi.right_band_powers.total() > 0.9,
    ...
    fn brain_types_differ_in_bilateral_output() {
        ...
        for &bt in BrainType::all() {
            ...
            assert!(bi.combined.band_powers.total() > 0.0,
                "{:?} produced zero band powers", bt);
        }

        // Aging should have weakest callosal coupling
        let aging_coupling = coupling_strengths.iter()
            .find(|(bt, _)| *bt == BrainType::Aging).unwrap().1;
        let normal_coupling = coupling_strengths.iter()
            .find(|(bt, _)| *bt == BrainType::Normal).unwrap().1;
        assert!(aging_coupling < normal_coupling,
            "Aging coupling ({}) should be less than Normal ({})",
            aging_coupling, normal_coupling);

        // ADHD should have weaker coupling than Normal
        let adhd_coupling = coupling_strengths.iter()
            .find(|(bt, _)| *bt == BrainType::Adhd).unwrap().1;
        assert!(adhd_coupling < normal_coupling,
        ...
        let ha_coupling = coupling_strengths.iter()
            .find(|(bt, _)| *bt == BrainType::HighAlpha).unwrap().1;
        assert!(ha_coupling >= normal_coupling,
    ...
    fn fhn_fires_from_bilateral_eeg() {
        ...
        assert!(result.firing_rate > 0.0,
            "FHN should fire from bilateral EEG, got rate={}", result.firing_rate);
    }
    ...
    fn callosal_delay_has_effect() {
        ...
        let diff: f64 = bi_normal.combined.eeg.iter()
            .zip(bi_zero.combined.eeg.iter())
            .map(|(a, b)| (a - b).abs())
            .sum::<f64>() / bi_normal.combined.eeg.len() as f64;

        assert!(diff > 1e-6,
            "Callosal delay should affect EEG output, got mean diff={}", diff);
    }
}
