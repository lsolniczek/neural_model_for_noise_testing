...
struct GreenPresetProvider: NoisePresetProvider {
    func setPreset(engine: NoiseEngine, movementController: MovementController) {
        ...
        // ── Showcase: Green ─────────────────────────────────────────────────
                    // Bandpass filtered at 500Hz. Narrow mid-range hum — sounds like
                    // a refrigerator or air duct. Drives WC(25) into beta range via
                    // gammatone band 1-2. The strongest beta driver in the model.
                    // Use sparingly (vol ≤ 0.20) to avoid beta lock at 73%+.
                    engine.setMasterGain(gain: 0.68)
                    engine.setAnchorColor(color: .green)
                    engine.setAnchorVolume(volume: 0.0)
                    engine.setAcousticEnvironment(environment: .forest)

                    engine.setObject(index: 0, active: true, color: .green,
                        x: 0.00, y: 0.5, z: 2.50, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 1, active: true, color: .green,
                        x: 2.17, y: -0.5, z: 1.25, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 2, active: true, color: .green,
                        x: 2.17, y: 0.5, z: -1.25, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 3, active: true, color: .green,
                        x: 0.00, y: -0.5, z: -2.50, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 4, active: true, color: .green,
                        x: -2.17, y: 0.5, z: -1.25, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 5, active: true, color: .green,
                        x: -2.17, y: -0.5, z: 1.25, volume: 0.20, reverbSend: 0.05)
