...
struct SSNPresetProvider: NoisePresetProvider {
    func setPreset(engine: NoiseEngine, movementController: MovementController) {
        ...
        // ── Showcase: SSN (Speech-Shaped Noise) ─────────────────────────────
                    // Shaped to match the long-term average speech spectrum.
                    // Concentrates 73% of gammatone energy in band 1 (200-800Hz)
                    // — the speech fundamentals range. Best neural carrier for
                    // dual-band goals (α+β) because it simultaneously feeds
                    // right-hemi JR alpha (via band 1) and left-hemi WC beta
                    // (via band 2). But causes cognitive fatigue over 5+ min
                    // due to involuntary speech parsing.
                    engine.setMasterGain(gain: 0.64)
                    engine.setAnchorColor(color: .ssn)
                    engine.setAnchorVolume(volume: 0.0)
                    engine.setAcousticEnvironment(environment: .anechoicChamber)

                    engine.setObject(index: 0, active: true, color: .ssn,
                        x: 0.00, y: 0.5, z: 2.50, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 1, active: true, color: .ssn,
                        x: 2.17, y: -0.5, z: 1.25, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 2, active: true, color: .ssn,
                        x: 2.17, y: 0.5, z: -1.25, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 3, active: true, color: .ssn,
                        x: 0.00, y: -0.5, z: -2.50, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 4, active: true, color: .ssn,
                        x: -2.17, y: 0.5, z: -1.25, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 5, active: true, color: .ssn,
                        x: -2.17, y: -0.5, z: 1.25, volume: 0.20, reverbSend: 0.05)
    }
}
