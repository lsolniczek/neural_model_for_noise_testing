struct WhitePresetProvider: NoisePresetProvider {
    func setPreset(engine: NoiseEngine, movementController: MovementController) {
        // ═══════════════════════════════════════════════════════════════════
                    ...
                    // ── Showcase: White ─────────────────────────────────────────────────
                    // Flat power spectrum. Equal energy at every frequency.
                    // Sounds bright and hissy — the "reference" noise.
                    // Feeds all gammatone bands equally. Model sees uniform
                    // tonotopic distribution (~25% per band).
                    engine.setMasterGain(gain: 0.7)
                    engine.setAnchorColor(color: .white)
                    engine.setAnchorVolume(volume: 0.0)
                    engine.setAcousticEnvironment(environment: .forest)

                    engine.setObject(index: 0, active: true, color: .white,
                        x: 0.00, y: 0.5, z: 2.50, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 1, active: true, color: .white,
                        x: 2.17, y: -0.5, z: 1.25, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 2, active: true, color: .white,
                        x: 2.17, y: 0.5, z: -1.25, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 3, active: true, color: .white,
                        x: 0.00, y: -0.5, z: -2.50, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 4, active: true, color: .white,
                        x: -2.17, y: 0.5, z: -1.25, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 5, active: true, color: .white,
                        x: -2.17, y: -0.5, z: 1.25, volume: 0.20, reverbSend: 0.05)
