struct BluePresetProvider: NoisePresetProvider {
    func setPreset(engine: NoiseEngine, movementController: MovementController) {
        ...
        // ── Showcase: Blue ──────────────────────────────────────────────────
                    // +3 dB/oct. Bright, airy, high-frequency emphasis. Sounds like
                    // digital static or a high-pressure air leak. Most energy sits
                    // above the gammatone filterbank's 80Hz LPF, making blue
                    // neurally near-invisible — the model barely sees it. Use for
                    // perceptual texture (rain, sparkle, air) without affecting
                    // the neural evaluation. The "free" color for spatial richness.
                    engine.setMasterGain(gain: 0.62)
                    engine.setAnchorColor(color: .blue)
                    engine.setAnchorVolume(volume: 0.0)
                    engine.setAcousticEnvironment(environment: .anechoicChamber)

                    engine.setObject(index: 0, active: true, color: .blue,
                        x: 0.00, y: 0.5, z: 2.50, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 1, active: true, color: .blue,
                        x: 2.17, y: -0.5, z: 1.25, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 2, active: true, color: .blue,
                        x: 2.17, y: 0.5, z: -1.25, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 3, active: true, color: .blue,
                        x: 0.00, y: -0.5, z: -2.50, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 4, active: true, color: .blue,
                        x: -2.17, y: 0.5, z: -1.25, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 5, active: true, color: .blue,
                        x: -2.17, y: -0.5, z: 1.25, volume: 0.20, reverbSend: 0.05)
    }
}
