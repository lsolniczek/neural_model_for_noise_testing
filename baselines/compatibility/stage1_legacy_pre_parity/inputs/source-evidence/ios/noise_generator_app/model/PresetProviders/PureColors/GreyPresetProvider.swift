...
struct GreyPresetProvider: NoisePresetProvider {
    func setPreset(engine: NoiseEngine, movementController: MovementController) {
        ...
        // ── Showcase: Grey ──────────────────────────────────────────────────
                    // Equal-loudness weighted (ISO 226). Perceptually flat — sounds
                    // like every frequency is equally loud to the ear. But in signal
                    // space, grey is LF-boosted to compensate for the ear's low-freq
                    // insensitivity. Dumps 88% of gammatone energy into band 0.
                    // Drives theta even harder than brown. Counterintuitive: the
                    // "flattest sounding" noise is the most theta-biased neurally.
                    engine.setMasterGain(gain: 0.7)
                    engine.setAnchorColor(color: .grey)
                    engine.setAnchorVolume(volume: 0.0)
                    engine.setAcousticEnvironment(environment: .forest)

                    engine.setObject(index: 0, active: true, color: .grey,
                        x: 0.00, y: 0.5, z: 2.50, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 1, active: true, color: .grey,
                        x: 2.17, y: -0.5, z: 1.25, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 2, active: true, color: .grey,
                        x: 2.17, y: 0.5, z: -1.25, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 3, active: true, color: .grey,
                        x: 0.00, y: -0.5, z: -2.50, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 4, active: true, color: .grey,
                        x: -2.17, y: 0.5, z: -1.25, volume: 0.20, reverbSend: 0.05)
                    engine.setObject(index: 5, active: true, color: .grey,
                        x: -2.17, y: -0.5, z: 1.25, volume: 0.20, reverbSend: 0.05)
