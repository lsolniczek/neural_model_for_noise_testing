...
# ── Architecture / contract (must match src/surrogate.rs) ───────────────────
...
class SurrogateMLP(nn.Module):
    """MLP with configurable hidden dims. The input dimension comes from the CSV
    contract generated by Rust (`g0..gN` + goal/brain/config metadata).
    """
...
def load_csv(path: Path) -> tuple[np.ndarray, np.ndarray, int]:
    """Load training data CSV. Expected columns:
    g0, g1, ..., gN, goal_id, brain_type_id, assr, thalamic_gate, cet, phys_gate, score
    """
    df = pd.read_csv(path)
    n_rows = len(df)
    ...
    for col in df.columns:
        match = re.fullmatch(r"g(\d+)", col)
        if match:
            genome_cols.append((int(match.group(1)), col))
    genome_cols.sort()
    if not genome_cols:
        raise ValueError("CSV does not contain any genome columns (expected g0..gN)")

    genome_indices = [idx for idx, _ in genome_cols]
    expected_indices = list(range(len(genome_cols)))
    if genome_indices != expected_indices:
        raise ValueError(
            "Genome columns must be contiguous from g0..gN; "
            f"got indices {genome_indices[:5]}...{genome_indices[-5:]}"
        )

    genome_col_names = [col for _, col in genome_cols]
    genome_dim = len(genome_col_names)
    input_dim = genome_dim + GOAL_DIM + BRAIN_TYPE_DIM + CONFIG_DIM
    print(f"  Surrogate contract: genome_dim={genome_dim}, input_dim={input_dim}")

    # Genome columns: g0..gN, normalized to [0, 1] per-column.
    ...
    genomes_raw = df[genome_col_names].values.astype(np.float64)
...
def train(X: np.ndarray, y: np.ndarray, epochs: int = 200, lr: float = 1e-3,
    ...
    model = SurrogateMLP(input_dim=X.shape[1], hidden_dims=hidden_dims)
...
def main():
    ...
    X, y, genome_dim = load_csv(args.csv_path)
